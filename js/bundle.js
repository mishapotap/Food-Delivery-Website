...
function calc() {
  // Calc
  const result = document.querySelector(".calculating__result span"); //Путь к результату
  ...
  } //Если в storage есть уже пол, то устанавливаем его в то же значение. Если нет - female по умолчанию и тоже его запоминаем
  ...
  } //Тоже самое только с ratio


  function initLocalSettings(selector, activeClass) {
    //Перебираем кнопки и когда она совпадет по значнию с той что находится в localStorage то на эту кнопку вешаем класс активности
    ...
    elements.forEach(elem => {
      ...
      } //Если female/male === female/male то добавляем класс активности


      if (elem.getAttribute("data-ratio") === localStorage.getItem("ratio")) {
        elem.classList.add(activeClass);
      } //Если 1.2/1.375/... === 1.2/1.375/... то добавляем класс активности
  ...
  function calcTotal() {
    if (!sex || !height || !weight || !age || !ratio) {
      ...
      return;
    } //Если не введена одна из переменных то текст по умолчанию ___ и выходим из функции calcTotal
    ...
    } //Установили мужскую и женскую формулу в результат
  ...
  function getDynamicInformation(selector) {
    //получаем рост вес возраст
    const input = document.querySelector(selector);
    input.addEventListener("input", () => {
      //проверяет чтобы в инпутах были цифры
  ...
  function bindPostData(form) {
    form.addEventListener("submit", event => {
      ...
      statusMessage.style.cssText = `
        ...
        margin: 0 auto;
      `;
      form.insertAdjacentElement("afterend", statusMessage); // Вставили кусок кода после элемента statusMessage

      const formData = new FormData(form); //Json берет данные с формы, превращает в массив с массивами, обратно в объект и его переводит в json формат который мы потом в postData постим на dj.json
...
function openModal(modalSelector, modalTimerId) {
  const modal = document.querySelector(modalSelector); // modal.classList.toggle("show");
...
function slider(_ref) {
  ...
  indicators.style.cssText = `
        ...
        list-style: none;
    `;
  slider.append(indicators); //Когда цикл отработает у нас сформируется определенное количесво точек
...
(__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill)();
 ...
 // Импортировали все функции


window.addEventListener("DOMContentLoaded", () => {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_4__.openModal)(".modal", modalTimerId), 50000); //Вызываем функции

  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_1__["default"])(".tabheader__item", ".tabcontent", ".tabheader__items", "tabheader__item_active");
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__["default"])(".timer", "2021-11-26");
